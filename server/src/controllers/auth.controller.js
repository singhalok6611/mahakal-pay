...
const AuthController = {
  ...
  async refresh(req, res) {
    ...
    const stored = await RefreshTokenModel.findValidByToken(refreshToken);
    if (!stored) {
      // Token reuse / revoked / unknown — kill all sessions for this user as a safety measure
      if (payload && payload.id) {
        await RefreshTokenModel.revokeAllForUser(payload.id);
      }
    ...
    const newStored = await RefreshTokenModel.findValidByToken(newRefreshToken);
    await RefreshTokenModel.revokeById(stored.id, newStored ? newStored.id : null);
  ...
  async forgotPassword(req, res) {
    ...
    if (user && user.status === 'active' && (!user.approval_status || user.approval_status === 'approved')) {
      try {
        const { rawToken } = await PasswordResetTokenModel.create({
          userId: user.id,
          ipAddress: req.ip,
          userAgent: req.headers['user-agent'],
        });
  ...
  async resetPassword(req, res) {
    ...
    const row = await PasswordResetTokenModel.findValidByRawToken(token);
    if (!row) {
    ...
    if (!user || user.status !== 'active') {
      ...
      await PasswordResetTokenModel.markUsed(row.id);
    ...
    try { await RefreshTokenModel.revokeAllForUser(user.id); } catch {}
  ...
  async changePassword(req, res) {
    ...
    const fullUser = await db.prepare('SELECT password_hash FROM users WHERE id = ?').get(req.user.id);
