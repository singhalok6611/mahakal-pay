...
import { getOperators, recharge, getTransactions } from '../../api/retailer.api';
...
export default function RetailerRecharge() {
  ...
  const [serviceType, setServiceType] = useState(searchParams.get('type') || 'mobile');
  ...
  useEffect(() => {
    getOperators({ service_type: serviceType })
      .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Recharge successful! New balance: ₹${res.data.balance.toFixed(2)}`);
      setForm({ operator: '', subscriber_id: '', amount: '' });
      refreshBalance();
      fetchTransactions();
    } catch (err) {
